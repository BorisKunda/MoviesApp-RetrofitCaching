...
//IN THIS EXAMPLE SETTINGS UNFINISHED AND DB CREATED TO STORE FAVOURITES
public class MainActivity extends AppCompatActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    int cacheSize = 10 * 1024 * 1024; // 10 MiB  -> maximum cache size .when it becomes full it refreshes (deletes all stored cache files).MiB is unit of measurement which quite similar to megabytes
    ...
    //Loads JSON with retrofit
    private void loadJSON(){

        try{
            if (BuildConfig.THE_MOVIE_DB_API_TOKEN.isEmpty()){//public static final String THE_MOVIE_DB_API_TOKEN = "88343a4758ad5bd50971e643e2f2b7de"; -> KEY
                Toast.makeText(getApplicationContext(), "Please obtain API Key firstly from themoviedb.org", Toast.LENGTH_SHORT).show();
                pd.dismiss();//pd -> progress dialog
                return;
            }
            //create cache object
            Cache cache = new Cache(getCacheDir(), cacheSize);//getCacheDir() ->grants cache files  path to the application specific cache directory on the filesystem.(its size defined in this example  by cacheSize variable)

            //Create OkHTTPClient object
            OkHttpClient okHttpClient = new OkHttpClient.Builder()
                    .cache(cache)
                    .addInterceptor(new Interceptor() {//Interceptor will intercept Retrofit's response(call)
                        @Override
                        public okhttp3.Response intercept(Interceptor.Chain chain)
                                throws IOException {//in case there will be exception it dealt further in code
